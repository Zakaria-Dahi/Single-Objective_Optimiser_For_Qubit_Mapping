...
def machine_selection(lb):
    ...
    for machine in backend_lists:
        ...
        qubits_num1 = backend.configuration().n_qubits;    
        configs.append(qubits_num1);
...
def layout_optimiser(iter,indiv,dim,pc,pm,shots,perc,e,backend,level,qubits_num0):
    ...
    pop = np.random.randint(0,qubits_num0,size=(indiv,dim))
    # repair function
    if dim == qubits_num0:
    ...
    else: # treats the case where the size of the circuit are less than the size of the machine
        for i in range(indiv): # browse all the chromosomes in the population 
            unique_val, counts = np.unique(pop[i], return_counts=True)
            ...
            while len(double_val_ind) > 0:
                ...
                pop[i]= repair2(pop[i],dim,qubits_num0,pick_list,unique_val,counts,double_val_ind)
                unique_val, counts = np.unique(pop[i], return_counts=True)
                double_val_ind = np.where(counts>1)
                double_val_ind = np.copy(double_val_ind[0])             
        ...
        for off in range(mat_pool_size):
            ...
            for index in range(dim):
                   if random.random() <= pm: # if condition of mutation satisfied perform the mutation
                       offspring1[index] = np.random.randint(0,qubits_num0) #apply the mutation           
            for index in range(dim):
                   if random.random() <= pm: # if condition of mutation satisfied perform the mutation
                       offspring2[index] = np.random.randint(0,qubits_num0) #apply the mutation           
            # repair offspring 1
            if dim == qubits_num0:
                ...
                unique_val, counts = np.unique(offspring1, return_counts=True)
                double_val_ind = np.where(counts>1)
                double_val_ind = np.copy(double_val_ind[0])
                # redundant_val = unique_val[double_val_ind]
                while len(double_val_ind) > 0:
                    ...
                    for indx10 in range(qubits_num0):
                        ...
                        if  check_val == False:
                            pick_list.append(indx10)
                    offspring1 = repair2(offspring1,dim,qubits_num0,pick_list,unique_val,counts,double_val_ind)
                    unique_val, counts = np.unique(offspring1, return_counts=True)
                    double_val_ind = np.where(counts>1)
                    double_val_ind = np.copy(double_val_ind[0])  
            # repair offspring 2
            if dim == qubits_num0:
                ...
                unique_val, counts = np.unique(offspring2, return_counts=True)
                double_val_ind = np.where(counts>1)
                double_val_ind = np.copy(double_val_ind[0])
                # redundant_val = unique_val[double_val_ind]
                while len(double_val_ind) > 0:
                    ...
                    for indx10 in range(qubits_num0):
                        ...
                        if  check_val == False:
                            pick_list.append(indx10)
                    offspring2 = repair2(offspring2,dim,qubits_num0,pick_list,unique_val,counts,double_val_ind)
                    unique_val, counts = np.unique(offspring2, return_counts=True)
                    double_val_ind = np.where(counts>1)
                    double_val_ind = np.copy(double_val_ind[0])  
            # evaluation
...
def fitness(individual,shots,backend,qubits_num2,level):
    circuit_test = QuantumCircuit(qubits_num2, qubits_num2)
    circuit_test.h(0)
    circuit_test.cx(0,range(1,qubits_num2))
    circuit_test.barrier()
    circuit_test.measure(range(qubits_num2), range(qubits_num2))
...
def fitness2(level,shots,backend,qubits_num3):
    circuit_test = QuantumCircuit(qubits_num3, qubits_num3)
    circuit_test.h(0)
    circuit_test.cx(0,range(1,qubits_num3))
    circuit_test.barrier()
    circuit_test.measure(range(qubits_num3), range(qubits_num3))
    sample_exec = [None]*shots; # store the circuit depth after each compilation 
    maxdepth = float('inf') # define and exagerated infinity
    for sh in range(shots):
        # circuit_test.draw(output='mpl') # to plot the circuit
        compiled_circuit = transpile(circuit_test, backend, layout_method = "noise_adaptive", optimization_level=level)
        # plot_circuit_layout(compiled_circuit, backend) # to plot the topology
        sample_exec[sh] = compiled_circuit.depth()
    ...
    layout_best_final = layout_best_config[:qubits_num3] # save only the qubits_num first, in case the circuit contains ancilla
...
def main():
    ...
    perc_entg = 1 # the percentage of qubits to be used.
    # backend_config = machine_selection(lb)
    backend_config = ['FakeManhattan']
    for machine in backend_config:
        ...
        dim = round(perc_entg * qubits_num); # size of the harware topology  = size of the chromosome
        """
        ...
        for e in range(exe):
             start =  time.time()
             exe_result_1 = layout_optimiser(iter,indiv,dim,pc,pm,shots,perc,e,backend,level,qubits_num)
             end = time.time()
        ...
        for e in range(exe):
             print("execution n°",e)
             start = time.time()
             exe_result_2 = fitness2(level,shots,backend,dim)
             end = time.time()
